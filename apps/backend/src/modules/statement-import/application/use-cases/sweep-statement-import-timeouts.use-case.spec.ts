...
describe("SweepStatementImportTimeoutsUseCase", () => {
  it("chama sweepTimeouts com os thresholds do ADR-0034 (60s csv/ofx, 20min pdf)", async () => {
    const { useCase, adminRepo } = make();

    await useCase.execute();

    expect(adminRepo.sweepTimeouts).toHaveBeenCalledWith("60 seconds", "20 minutes");
  });
