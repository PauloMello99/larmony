...
| M14 | Billing (Stripe) + Entitlements | [15-billing-entitlements.md](../docs/product/features/15-billing-entitlements.md) | **✅ entregue (2026-07-12, ADR-0026)** — todo o backlog B-1..B-7 mesclado. Estende o shell `subscriptions` (colunas de coupon/comp, sem migração de enum — `type='custom'` já cobria isenção); RLS já era super_admin-only para escrita. Super-admin concede desconto (via Stripe Coupon, API) ou isenção total (100% local) **sem abrir o dashboard do Stripe**; `EntitlementsService` como gating server-side único. Backlog: B-1 (migration) → B-2 (checkout/portal) → B-3 (webhook+reconciliação) → B-4 (entitlements aplicado) → B-6 (frontend/paywall) → B-7 (admin comp/desconto). **Pendência de produto: D-1** (régua final das capabilities do Free — só `advanced_reports` gateado por ora). Deploy: registrar o webhook + `whsec` real em staging/prod. | [ADR-0026](adr/0026-billing-stripe-entitlements.md) |
