...
| M15 | Redesign do super admin + suporte | — | **🔨 em andamento (2026-07-14)** — 3 PRs: **PR 1 ✅ core** (admin centrado em lares: listas server-side com filtro por e-mail do dono/plano; drill-down em 6 abas incl. finanças read-only e notificações; suspensão de lar pago cancela no Stripe com prorate antes — 409 sem o flag; promote/demote de super_admin removido → DB-only via runbook; primeiro e2e do AdminController, 28 casos). **PR 2 ✅ billing KPIs** (`GET /admin/stats/billing` local + espelho mínimo de `invoice.paid`/`invoice.payment_failed` em `billing_invoice_events` — idempotente em 2 camadas, sem dado de cartão — alimentando `revenueCents30d`/`failedPayments30d` reais; deep-links Stripe de customer+subscription na aba Assinatura e "Ver receita no Stripe" no overview; staging/prod precisam do `enabled_events` do webhook atualizado manualmente, ver `docs/billing-local-testing.md`). **PR 3 pendente**: canal de suporte (tickets in-app threaded + inbox no admin + e-mail via Resend). Follow-ups: feature flags (ADR-0009), PLAT-3 | [ADR-0028](adr/0028-admin-redesign-households-centric.md) |
