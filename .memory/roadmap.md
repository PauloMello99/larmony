---
name: roadmap
description: Roadmap do Larmony — v1 completo (M1–M9 + ADR-0020); v1.1 completo (M10 ✅, M11 ✅, M12 ✅); M13 Open Finance em espera com gatilho (ADR-0025); M14 Billing/Entitlements ✅ (ADR-0026); M15 Redesign admin+suporte ✅ (ADR-0028); M16 Assinatura pago-only 2 tiers ✅ (ADR-0029)
metadata:
  type: project
---
...
| M13 | Open Finance (conexão bancária) | [14-open-finance.md](../docs/product/features/14-open-finance.md) | **⏸️ EM ESPERA COM GATILHO (2026-07-11, ADR-0025)** — desacoplado do lançamento: discovery da Fase 0 mostrou piso fixo de agregador (Pluggy R$ 2,5k/mês; Belvo ~R$ 6k; Tecnospeed R$ 540 + adesão) inviável com base zero; integração técnica em si é rápida (~3–5 sem) e compatível com o repo. Ativa com ~150–200 lares Premium OU waitlist do plano Conectado (tier separado ~R$ 29,90–44,90, nunca embutido no Premium R$ 14,90). Consentimento hoje é por **prazo indeterminado** (Res. Conj. 7/2023 — premissa de 12 meses da spec corrigida). Ponte opcional: importação CSV/OFX no Premium. Pré-requisito: entitlements/billing (Stripe) — **próximo milestone técnico do lançamento** | [ADR-0025](adr/0025-open-finance-desacoplado-lancamento.md) |
| M14 | Billing (Stripe) + Entitlements | [15-billing-entitlements.md](../docs/product/features/15-billing-entitlements.md) | **✅ entregue (2026-07-12, ADR-0026)** — todo o backlog B-1..B-7 mesclado. Estende o shell `subscriptions` (colunas de coupon/comp, sem migração de enum — `type='custom'` já cobria isenção); RLS já era super_admin-only para escrita. Super-admin concede desconto (via Stripe Coupon, API) ou isenção total (100% local) **sem abrir o dashboard do Stripe**; `EntitlementsService` como gating server-side único. Backlog: B-1 (migration) → B-2 (checkout/portal) → B-3 (webhook+reconciliação) → B-4 (entitlements aplicado) → B-6 (frontend/paywall) → B-7 (admin comp/desconto). **D-1 resolvido (2026-07-13, adendo ADR-0026)**: régua final do Free — 1 lar (owner), 2 membros/lar, 0 categorias personalizadas, 3 metas, 3 orçamentos ativos, sem relatórios avançados nem export CSV; Family = ilimitado em tudo. Deploy: registrar o webhook + `whsec` real em staging/prod. | [ADR-0026](adr/0026-billing-stripe-entitlements.md) |
| M15 | Redesign do super admin + suporte | — | **✅ entregue (2026-07-14)** — 3 PRs: **PR 1 core** (admin centrado em lares: listas server-side com filtro por e-mail do dono/plano; drill-down em 6 abas incl. finanças read-only e notificações; suspensão de lar pago cancela no Stripe com prorate antes — 409 sem o flag; promote/demote de super_admin removido → DB-only via runbook; primeiro e2e do AdminController, 28 casos). **PR 2 billing KPIs** (`GET /admin/stats/billing` local + espelho mínimo de `invoice.paid`/`invoice.payment_failed` em `billing_invoice_events` — idempotente em 2 camadas, sem dado de cartão — alimentando `revenueCents30d`/`failedPayments30d` reais; deep-links Stripe de customer+subscription na aba Assinatura e "Ver receita no Stripe" no overview; staging/prod precisam do `enabled_events` do webhook atualizado manualmente, ver `docs/billing-local-testing.md`). **PR 3 canal de suporte** (tickets in-app threaded `support_tickets`/`support_ticket_messages`, sem RLS; usuário abre/lista/responde os próprios — reply reabre `answered`→`open`; admin vê todos, responde — `answered` + notifica autor — e fecha/reabre; fechado bloqueia resposta com 409; criar ticket notifica todos os super_admin in-app+e-mail; nav do admin com badge de abertos). 166 e2e / 124 unit verdes ao final. Follow-ups: feature flags (ADR-0009), PLAT-3, ferramenta de suporte externa (segue simples/manual por ora) | [ADR-0028](adr/0028-admin-redesign-households-centric.md) |
| M16 | Assinatura pago-only: 2 tiers + trial self-serve | — | **✅ entregue (2026-07-14)** — 4 PRs, fim do freemium (supersede o adendo D-1 do ADR-0026). **PR 0**: subagente `pricing-strategist` pesquisa concorrentes BR e recomenda preço — Essencial R$ 9,90/mês (R$ 99/ano) e Completo R$ 19,90/mês (R$ 199/ano), aprovados pelo responsável. **PR 1**: `ResolvedPlan` vira `locked\|essencial\|completo`; régua por contagem removida (pago = ilimitado, só capabilities); `ActiveSubscriptionGuard` bloqueia escrita (402) por-controller, não global (evita vetor de escrita pré-auth); `households` fora do guard. **PR 2**: checkout com seleção de plano (`planKey`) + trial self-serve de 30 dias, cartão upfront; onboarding leva direto ao gate de assinatura. **PR 3**: `LockedBanner` de somente-leitura; orçamentos/lançamentos programados com paywall de página inteira; landing 2 planos + trial, 7 locales. **PR 4**: listagem de faturas no admin (`listInvoices`); trial administrativo local removido (redundante — self-serve via Stripe); painel mostra o `tier`. Enum do banco não mudou. Unit/e2e verdes em cada PR; smoke real no browser (checkout Stripe test mode) | [ADR-0029](adr/0029-subscription-paid-only-2-tiers.md) |
